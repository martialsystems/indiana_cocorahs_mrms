# Copyright (c) 2026 Martial Systems LLC

import sys
from pathlib import Path

import pytest

REPO = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(REPO))

from qpeforge._bootstrap import ensure_paths

ensure_paths()

from graphforge.product_law import LawBlockedError

from qpeforge.gate import require_claims, require_fetch, require_no_p_sfha, require_split
from qpeforge.product_laws import laws


def test_no_p_sfha() -> None:
    require_no_p_sfha(thread_id="t.p.ok")
    with pytest.raises(LawBlockedError):
        require_no_p_sfha(p_sfha_feature=True, thread_id="t.p.bad")


def test_spatial_split() -> None:
    require_split(thread_id="t.s.ok")
    with pytest.raises(LawBlockedError):
        require_split(random_split=True, thread_id="t.s.rand")
    with pytest.raises(LawBlockedError):
        require_split(station_leak=True, thread_id="t.s.leak")
    with pytest.raises(LawBlockedError):
        require_split(august_2026_in_train=True, thread_id="t.s.aug")
    with pytest.raises(LawBlockedError):
        require_split(gauge_as_feature=True, thread_id="t.s.gauge")


def test_fetch_radar_only() -> None:
    require_fetch(
        cocorahs_ok=True,
        mrms_ok=True,
        product_is_radar_only=True,
        thread_id="t.f.ok",
    )
    with pytest.raises(LawBlockedError):
        require_fetch(cocorahs_ok=True, mrms_ok=False, product_is_radar_only=True, thread_id="t.f.mrms")
    with pytest.raises(LawBlockedError):
        require_fetch(
            cocorahs_ok=True,
            mrms_ok=True,
            product_is_radar_only=True,
            product_is_gauge_corr=True,
            thread_id="t.f.gc",
        )
    with pytest.raises(LawBlockedError):
        require_fetch(
            cocorahs_ok=True,
            mrms_ok=True,
            product_is_radar_only=True,
            substitute_stageiv=True,
            thread_id="t.f.iv",
        )


def test_claims_and_figure_cap() -> None:
    require_claims(n_figures=2, thread_id="t.c.ok")
    with pytest.raises(LawBlockedError):
        require_claims(n_figures=3, thread_id="t.c.fig")
    with pytest.raises(LawBlockedError):
        require_claims(bias_as_wet_mask=True, thread_id="t.c.wet")
    with pytest.raises(LawBlockedError):
        require_claims(gaugecorr_independent=True, thread_id="t.c.gc")


def test_registry() -> None:
    assert {row["id"] for row in laws()} == {
        "qpe.no_p_sfha",
        "qpe.spatial_split",
        "qpe.fetch_radar_only",
        "qpe.claim_bans",
    }
